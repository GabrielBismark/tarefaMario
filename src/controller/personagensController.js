...
router.get('/personagens/add', (req, res) => {
    res.render("admin/personagens/personagens");
});
...
router.get('/editar_personagens/:id', (req, res) => {
    Personagens.findOne({ _id: req.params.id }).lean().then((personagens) => {
        res.render("admin/personagens/editpersonagens", { personagens: personagens });
    });
});
/*6°) Recebe as informações do botão que está no editpersonagens.handlebar
