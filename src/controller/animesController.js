...
/*_____________ Rotas dos animes __________________ */
...
router.get('/editar_animes/:id', (req, res) => {
    Animes.findOne({ _id: req.params.id }).lean().then((animes) => {
        res.render("admin/animes/editanimes", { animes: animes });
    });
});
/*6°) Recebe as informações do botão que está no editanimes.handlebar
...
router.post('/animes/editar_animes', (req, res) => {
    Animes.updateOne({ _id: req.body._id },
        {
            $set: {
                nome: req.body.nome, genero: req.body.genero
            }
        }).then(() => {
            res.redirect("/rota_animes/animes");
        });
